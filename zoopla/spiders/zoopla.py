...
class ZooplaSpider(scrapy.Spider):
    ...
    def parse(self, response: HtmlResponse, **kwargs):
        if response.status == 200:
            ...
            if listing_tag.text.lower() in ("currently for sale", "currently for rent") and sale_description.children:
                links = map(
                ...
                if links:
                    title = html.find("h1", class_="e1e889nu0")
                    if title:
                        yield {"title": title.text, "links": "\n".join(list(links))}
            else:
                pass
        else:
            pass
